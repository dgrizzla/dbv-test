...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_sel_png_usuario_busqueda`(IN `pidusuario` INT(11))
    NO SQL
SELECT bu.id, bu.producto, bu.descripcion, bu.id_cat, bu.id_subcat, bu.id_ssubcat, bu.id_estado, bu.id_img_destacada, bu.precio_del, bu.precio_al, bu.fecha_ingreso, bu.fecha_limite,  dep.nombre, ui.url_img, ui.orden, bu.id_gps, cat.nombre as nombre_categoria
